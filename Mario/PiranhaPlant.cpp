...
void PiranhaPlant::HandleTimeSwitchState()
{
	if (state == PIRANHAPLANT_STATE_BITING &&
		GetTickCount64() - switchTime > PIRANHAPLANT_SWITCHING_STATE_TIME
		&& switchTime != 0)
	{
		switchTime = 0;
	...
	if (state == PIRANHAPLANT_STATE_DEATH &&
		GetTickCount64() - deathTime > PIRANHAPLANT_INACTIVE_TIME
		&& deathTime != 0)
	{
